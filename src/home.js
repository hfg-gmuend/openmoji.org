$(document).ready(function() {
	...
	function genEmojiCloud() {
		...
		for(var i = 0; i < EMOJI_POSITIONS.length; i++) {
			...
			// add emoji to html
			if( !(xPos >= -5 && xPos <= 105 && yPos >= 0 && yPos <= 100) ) $( "#landing .content" ).append("<a href='https://openmoji.org/library#emoji=" + shuffledList[i].hexcode + "'><img class='emoji lazy' data-src='data/color/svg/" + shuffledList[i].hexcode + ".svg' align='middle' style='top: " + xPos + "%; left: " + yPos + "%'></a>");
		}
	}
	...
	function showCategoryExamples( category ) {
		...
		for(var i = 0; i < EXAMPLES_COUNT; i++) {
			...
			html += "<a class='categories-item' href='https://openmoji.org/library#emoji=" + filteredList[i].hexcode + "'><img src='data/color/svg/" + filteredList[i].hexcode + ".svg'></a>";
		}
		html += "</div>";
	...
	//------------ Event listeners ------------
	// search field listener to change location to https://openmoji.org/library and set search filter
	console.log(window.location);
	$( ".search" ).keydown(function(e) {
		var url = window.location.href;
		if (e.which == 13) {
			window.location.href = url.substring(0, url.lastIndexOf("/") + 1) + "https://openmoji.org/library#search=" + $( this ).val();
		}
	});
